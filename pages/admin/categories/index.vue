<template>
  <v-container>
    <custom-appbar title="Categories" :prominent="false"> </custom-appbar>
    <v-data-table
      :headers="headers"
      :search="search"
      :items="categories"
...
import CategoryCreate from '../../../components/forms/category-create.vue'
...
export default {
  ...
  components: { customAppbar, CustomAlert, CategoryCreate },
  ...
  methods: {
    editItem(category) {
      this.$router.push({
        name: 'admin-jobs-categories-category',
        params: { category: category },
      })
    },

    deleteItem(id) {
      this.$store.dispatch('categories/removeCategory', id).then(() => {
