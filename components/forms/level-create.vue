  ...
  <v-dialog v-model="dialog" width="500">
    ...
    <v-card>
      <v-card-title class="text-h5">
        <v-icon class="mr-3" color="#69BF88">mdi-poll</v-icon>
        Create Level</v-card-title
      >
        ...
        <v-row>
          <v-col>
            <v-text-field
              ...
              @blur="$v.level.$touch()"
              @input="$v.level.$touch()"
              :error-messages="validation('required', $v.level)"
              color="#69BF88"
              outlined
              clearable
              dense
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.$v.$anyErrors) {
        ...
        this.$store.dispatch('levels/storeLevel', {
          level: this.level,
        })
        this.reset
      }
